...
name: publish
description: >-
  Publish the octop-harness Python package: cut a release branch from develop, bump
...
自动化 octop-harness（PyPI: `octop-harness`）的完整发布流程。
...
| `PYPI_PACKAGE` | `octop-harness` | PyPI 包名（仅用于提示文案） |
...
2. `README_FILE` / `README_CN_FILE`（若存在）：
   ...
   - 若仅使用动态 `shields.io/pypi/v/octop-harness` 徽标，**无需改徽标**（PyPI 发布后自动更新），但仍须检查 README 中是否有硬编码安装示例版本（如 `pip install octop-harness==X.Y.Z`）并同步。
...
```bash
gh pr create \
  --repo TencentCloud/octop-harness \
  --base {TARGET_BRANCH} \
...
- [ ] Release workflow publishes octop-harness to PyPI and syncs main → develop
EOF
)"
```
