...
- smoke-test a server with the FastMCP CLI before wiring it into octop-harness or another MCP client
...
Use the `mcporter` skill when the server already exists and only needs CLI access. For octop-harness runtime integration, add the server to `mcp_server_configs` in your agent config.
...
When the goal is octop-harness integration, add the server to `mcp_server_configs` in your deployment config, or keep using FastMCP CLI commands during development until the interface stabilizes.
...
### octop-harness cannot see the deployed server
